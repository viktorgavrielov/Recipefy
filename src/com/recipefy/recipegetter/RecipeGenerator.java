...
public class RecipeGenerator {
	...
	public static enum FilterValue {ALLOWED_INGREDIENT, EXCLUDED_INGREDIENT, ALLOWED_ALLERGY, ALLOWED_DIET,
		...
		private List<RecipeData> makeRecipeData(String searchResponseJson) throws JsonParseException, IOException {
			...
			BitmapGetter[] bmgarray = new BitmapGetter[Integer.parseInt(MAX_RESULTS)];
			int bmgarrayi = 0;
			...
			for(JsonNode recipe : rootNode.path("matches")){
				...
				for(JsonNode smallImageUrl : recipe.path("smallImageUrls")){
					if(!smallImageUrl.isMissingNode()){
						BitmapGetter bmg = new BitmapGetter(recipeData, smallImageUrl.toString().replace("\"", ""));
						bmgarray[bmgarrayi] = bmg;
						bmgarrayi ++;
						bmg.start();
						break;
					}
		 ...
		 * @return
		 */
		private class BitmapGetter extends Thread{
			
			RecipeData recipeData;
			String src;
			private BitmapGetter(RecipeData recipeData, String src){
			...
			private void getBitmapFromURL() {
				try {
					...
					recipeData.setImageSmall(myBitmap);
				} catch (IOException e) {
					return;
